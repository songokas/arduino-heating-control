...
int xxtea_setup(const uint8_t *key, size_t len)
{
...
int xxtea_encrypt(const uint8_t *data, size_t len, uint8_t *buf, size_t *maxlen)
{
  int ret = XXTEA_STATUS_GENERAL_ERROR;
  int32_t l;
  do{
...
int xxtea_decrypt(uint8_t *data, size_t len, uint8_t * output, size_t outputLen)
{
  int ret = XXTEA_STATUS_GENERAL_ERROR;
  int32_t l;
  do {
    if(data == NULL || len <= 0 || (len%4) != 0 || outputLen > len)
    {
      ret = XXTEA_STATUS_PARAMETER_ERROR;
      break;
    }
    ...
    dtea_fn1(xxtea_data, l, (const uint32_t *)xxtea_key);
    // Copy Encrypted Data back to buffer
    memcpy((void *)output, (const void *)xxtea_data, outputLen);
    ret = XXTEA_STATUS_SUCCESS;
  }while(0);
  return ret;
}
...
String xxtea_c::decrypt(String data)
{
  ...
  if(this->keyset && data.length() != 0 && (data.length() % 4) == 0)
  {
    ...
    if(data.length() < (MAX_XXTEA_DATA8 * 2))
    {
      ...
      // Perform Decryption
      if(xxtea_decrypt(this->data, len, this->data, len) == XXTEA_STATUS_SUCCESS)
      {
        String result;
